...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(80)
        self.cell(30, 10, 'Basic Multiplication Worksheet', align='C')
        self.ln(30)
...
def generate_random_problems(num_problems, min_value=1, max_value=20):
    """Generates random multiplication problems and their solutions."""
    problems = []
    solutions = []
...
def generate_multiplication_worksheet(num_problems=10, include_answer_key=False, output_path="basic_multiplication.pdf"):
    pdf = PDF()
    pdf.alias_nb_pages()
    pdf.add_page()
    ...
    pdf_path = os.path.join(output_dir, 'basic_multiplication.pdf')
    #pdf.output(pdf_path)
    pdf.output(output_path)
    
# To generate a PDF
#generate_multiplication_worksheet(include_answer_key=True)