...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(80)
        self.cell(30, 10, 'Negative Multiplication Worksheet', align='C')
        self.ln(30)
...
def generate_random_problems(num_problems, min_value=-20, max_value=20):
    """Generates random negative multiplication problems with positive and negative numbers."""
    problems = []
    solutions = []
...
def generate_negative_multiplication_worksheet(num_problems=10, include_answer_key=False, output_path="negative_multiplication.pdf"):
    pdf = PDF()
    pdf.alias_nb_pages()
    pdf.add_page()
    ...
    pdf_path = os.path.join(output_dir, 'negative_multiplication.pdf')
    #pdf.output(pdf_path)
    pdf.output(output_path)
        
# To generate a PDF
#generate_negative_multiplication_worksheet(include_answer_key=True)