...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(80)
        self.cell(30, 10, 'Negative Division Worksheet', align='C')
        self.ln(30)
...
def generate_random_problems(num_problems, min_value=-20, max_value=20):
    """Generates random negative division problems with positive and negative numbers."""
    problems = []
    solutions = []
...
def generate_negative_division_worksheet(num_problems=10, include_answer_key=False, output_path="negative_division.pdf"):
    pdf = PDF()
    pdf.alias_nb_pages()
    pdf.add_page()
